...
import { Search, TrendingUp, Wallet, LogOut, ExternalLink, GitBranch, Loader2 } from "lucide-react"
...
export default function Page() {
  ...
  const fetchPullRequests = async () => {
    try {
      setPrLoading(true)
      setPrError(null)
      const response = await fetch("https://stipendless-kameron-programmatic.ngrok-free.dev/api/contributor/installationid")
                      ...
                      >
                        <CardContent className="p-6">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <span className="text-sm font-mono text-gray-400">{pr.repo_name}</span>
                                <Badge
                                  variant="secondary"
                              ...
                              <h3 className="text-lg font-bold text-white mb-2">{pr.title}</h3>
                              <div className="flex flex-wrap gap-2 text-sm text-gray-400">
                                <span>PR #{pr.github_pr_number}</span>
                                <span>•</span>
                                <span>Complexity: {pr.complexity_score}/10</span>
                                {pr.merged_at && (
                                  <>
                                    <span>•</span>
                            ...
                            <div className="flex flex-col items-end gap-3">
                              <div className="text-right">
                                ...
                                <p className="text-gray-500 text-xs">{pr.actual_payout ? "Paid" : "Estimated"}</p>
                              </div>
                              <a
